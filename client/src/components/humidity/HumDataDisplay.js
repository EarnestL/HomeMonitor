...
import HumChartCard from './HumChartCard';
import HumTableCard from './HumTableCard';
import ChartPlaceHolder from '../universal/ChartPlaceHolder';
import TablePlaceHolder from '../universal/TablePlaceHolder';

function HumDataDisplay() {
    const [initialData, setInitialData] = useState([]);
    const [loading, setLoading] = useState(true);
    const [filter, setFilter] = useState('today');

    const apiEndpoint = `${process.env.REACT_APP_API_BASE}/humidity?${
        ...
        <div className="w-full h-full">
            ...
            <div className="flex flex-col lg:flex-row w-full h-full gap-6">
                {loading ? (
                    ...
                    <>
                        <div className="w-full lg:w-2/3 h-full">
                            <HumChartCard initialData={initialData} />
                        </div>
                        <div className="w-full lg:w-1/3 h-full">
                            <HumTableCard initialData={initialData} />
                        </div>
                    </>
                )}
            </div>
